...
def create_tf_record(data, label_map_dict, ignore_difficult_instances=False):
    """
...
def main(_):
    ...
    for dataset in dataset_list:
        ...
        for example in examples_list:
            ...
            root_ext_pair = os.path.splitext(data['path'])
            data['path'] = os.path.join(dataset, example + root_ext_pair[1])
            data['path'] = create_jpg_imgs(data['path'])

            tf_example = create_tf_record(data, label_map_dict, ignore_difficult_instances=FLAGS.ignore_difficult_instances)
